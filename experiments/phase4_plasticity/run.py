...
def _load_genomes(source_dir: str) -> list[Genome]:
    ...
    if not os.path.exists(genome_path):
        raise FileNotFoundError(
            ...
            "Run a phase4 evolution stage first."
        )
...
def _care_window_metrics(care_records, population_history: list[int], window_end: int) -> dict:
    ...
    return {
        ...
        "phase2_window_baseline":        0.076,   # phase3-genomes, no plasticity, ticks 0-120
...
def _run_evolution(seed: int, kin_conditional: bool) -> str:
    ...
    print(f"\n[phase4 | {stage_name}] Output: {output_dir}")
...
def _run_zeroshot(seed: int, source_dir: str, kin_conditional: bool) -> str:
    ...
    save_metadata(
        ...
        note=(
            ...
            "Compare care_window rate to phase2 baseline (0.076)."
            if kin_conditional else
    ...
    metrics = {
        ...
        "care_window":              window,
        "phase2_baseline_all":      0.0144,
        "phase2_baseline_window":   0.076,
        "note": (
    ...
    print(f"  Care/m-tick (window 0-{config.maturity_age})   : {window['care_per_mother_tick_in_window']:.5f}  [fair — compare to 0.07633]")
    return output_dir
