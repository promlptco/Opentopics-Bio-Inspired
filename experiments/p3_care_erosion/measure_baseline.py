...
PHASE_NAME = "phase05_zeroshot_standard"


def load_evolved_genomes(source_run_dir: str) -> list[Genome]:
    """Load top genomes from Phase 3 (maternal evolution) output."""
    genome_path = os.path.join(source_run_dir, "top_genomes.json")
    
    if not os.path.exists(genome_path):
        print(f"Warning: {genome_path} not found. Run phase04_care_erosion evolution stage first.")
...
def run(seed: int = 42, source_run_dir: str = None):
    # 1. Load evolved genomes first — init_mothers matches genome count
    if source_run_dir:
        genomes = load_evolved_genomes(source_run_dir)
        source_genomes = source_run_dir
    else:
        genomes = None
    ...
    save_metadata(
        ...
        note="Evolved genomes from phase04 care erosion evolution — zero-shot transfer test",
    )
    ...
    print(f"\n[phase05_zeroshot_standard] Output: {output_dir}")
...
if __name__ == "__main__":
    ...
    # run(seed=42, source_run_dir="outputs/phase04_care_erosion/run_xxx")
    run(seed=42)