...
Locked parameters (biological / ecological constraints):
  ...
  eat_gain = 0.2                        (food entity energy, equal for mother and child)
  care_weight = forage_weight = self_weight = 1.0  (unbiased motivation)
  Percept8 BALANCED ecology             (perception_radius=8, move_cost=0.005, hunger_rate=1/35)
...
def load_sweep_config(init_food: int, seed: int = 42) -> Config:
    """Build Phase 3 Config with percept8 BALANCED ecology and given init_food."""
    with open(_PHASE2_JSON) as f:
        data = json.load(f)
    ec = data["balanced"]["selected_config"]

    return Config(
        # Ecology — percept8 BALANCED (do not change)
