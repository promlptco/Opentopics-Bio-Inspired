...
Stages:
  ...
  'zeroshot'      — 5c: load evolved genomes, run 1 generation (no reproduction, no mutation).
                    Measures care_window rate vs Phase 3 baseline (0.09069).
...
def _load_genomes(source_dir: str) -> list[Genome]:
    ...
    if not os.path.exists(genome_path):
        raise FileNotFoundError(
            ...
            "Run phase5 evolution stage first."
        )
...
def _care_window_metrics(care_records, population_history: list[int], window_end: int) -> dict:
    ...
    return {
        ...
        "phase3_zeroshot_baseline":       PHASE3_ZS_BASELINE,
        "note": "Compare to phase3 zero-shot window rate 0.09069 for assimilation test.",
    }
...
def _run_evolution(seed: int, stage: str = "evolution",
    ...
    print(f"  Selection gradient r  : {grad:.4f}  (Phase 3 baseline: {PHASE3_SELECTION_R})"
