#!/usr/bin/env python3
"""
Phase 5b -- Food Ecology Calibration with Reproduction ON

Sweeps init_food x food_replenish_amount to find the weakest food ecology
where canonical care can sustain multi-generational ecology without extinction.

Food sweep (4 x 3 = 12 ecologies):
  init_food            = [45, 60, 80, 100]
  food_replenish_amount = [5, 10, 15]
...
Usage:
    python experiments/phase5b_food_ecology/run.py --duration 3000 --seeds 30
"""
...
INIT_FOOD_VALUES  = [45, 60, 80, 100]
REPLENISH_AMOUNTS = [5, 10, 15]
...
# Colors for ecology lines (12 ecologies)
_ECO_COLORS = [
    ...
    for t in range(duration):
        ...
        for r in results:
            if len(r["per_tick"]) > t:
                ...
                # seed extinct before this tick (shouldn't happen with zero-padding, but guard)
...
def select_ecology(all_agg: list[dict]) -> dict:
    ...
    Searches progressively relaxed thresholds until a match is found.
    Returns the canonical aggregate dict for the selected ecology.
    """
    canonical_by_eco = {
    ...
    for _tier, ext_threshold, gen2_min in criteria:
        for a in sorted_canonical:
            ...
            # No-care must have higher extinction rate than canonical
            if nc.get("extinction_rate", 1.0) < a["extinction_rate"]:
                continue
            return a

    # Fallback: return ecology with lowest canonical extinction rate
...
def save_config_json(duration: int, n_seeds: int, out_dir: str) -> None:
    doc = {
        "phase": "5b",
        "experiment": "food_ecology_calibration",
...
def save_summary_json(all_agg: list[dict], selected: dict, out_dir: str) -> str:
    ...
    if any_canonical_viable and no_care_worse and feed_events_occur and care_improves:
        verdict = "PASS -- viable food ecology found; canonical care survives and outperforms no-care"
    elif any_canonical_viable:
        verdict = "PASS (partial) -- viable ecology found but no-care contrast weaker than expected"
    else:
        verdict = "FAIL -- no food ecology supports canonical care persistence by tick " + str(
            all_agg[0].get("n_seeds", 0)
        )
    ...
    doc = {
        ...
        "checks": {
            ...
            "population_not_extinct":          pop_viable,
        },
        "selected_ecology": {
...
def plot_pop_over_time_canonical(all_per_tick: dict, out_dir: str, duration: int) -> None:
    ...
    for i, ek in enumerate(eco_list):
        ...
        ax.plot(ticks, pop_arr, color=_ECO_COLORS[i % len(_ECO_COLORS)],
                label=f"F={ifv} R={ra}", linewidth=1.4, alpha=0.85)
    ax.set_xlabel("Tick")
    ax.set_ylabel("Total Population (mean, 30 seeds incl. extinct=0)")
    ax.set_title(
        f"Phase 5b: Population Over Time — Canonical Care (cw=0.3)\n"
        f"All 12 food ecologies | reproduction=ON | {duration} ticks"
    )
...
def plot_final_pop_heatmap(all_agg: list[dict], out_dir: str) -> None:
    """Plot 02: final population heatmap (canonical)."""
    arr = _build_heatmap_array(all_agg, "canonical", "final_pop_mean")
    fig, ax = plt.subplots(figsize=(7, 4))
    _draw_heatmap(ax, arr, "Final Population Mean — Canonical (cw=0.3)",
                  "Blues", vmin=0.0, vmax=float(np.nanmax(arr)) + 0.1, fmt=".1f")
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "plots", "02_finalpop_heatmap.png"), dpi=150)
    plt.close()


def plot_extinction_heatmap(all_agg: list[dict], out_dir: str) -> None:
    """Plot 03: extinction rate heatmap (canonical)."""
    arr = _build_heatmap_array(all_agg, "canonical", "extinction_rate")
    fig, ax = plt.subplots(figsize=(7, 4))
    _draw_heatmap(ax, arr, "Extinction Rate — Canonical (cw=0.3)\n(0.0 = all survive, 1.0 = all extinct)",
                  "RdYlGn_r", vmin=0.0, vmax=1.0, fmt=".2f")
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "plots", "03_extinction_heatmap.png"), dpi=150)
    plt.close()
...
def plot_food_count_over_time(all_per_tick: dict, out_dir: str, duration: int) -> None:
    ...
    ax.set_title(
        f"Phase 5b: Food Count Over Time — Canonical Care\n"
        f"All 12 food ecologies | {duration} ticks"
    )
    ax.legend(fontsize=8, ncol=3, loc="upper right")
    ax.set_xlim(0, duration)
    ax.set_ylim(bottom=0)
...
def plot_births_deaths_over_time(all_per_tick: dict, sel_eco_key: str,
    ...
    """Plot 06: births and deaths per tick for all 3 conditions under selected ecology."""
    ...
    fig.suptitle(
        f"Phase 5b: Births & Deaths Over Time — Selected Ecology "
        f"(init_food={ifv}, replenish={ra})",
        fontsize=11,
    )
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "plots", "06_births_deaths_over_time.png"), dpi=150)
    plt.close()
...
def plot_child_survival_by_condition(all_agg: list[dict], sel_init_food: int,
    ...
    bars1 = ax.bar(x - width / 2, init_surv,    width, label="Init Child Survival",    color=[c + "cc" for c in colors], edgecolor="black", linewidth=0.6)
    bars2 = ax.bar(x + width / 2, overall_surv, width, label="Overall Child Survival", color=colors,                       edgecolor="black", linewidth=0.6, alpha=0.75)
    ...
    ax.set_title(
        f"Phase 5b: Child Survival by Condition\n"
...
def plot_condition_comparison_selected(all_per_tick: dict, all_agg: list[dict],
    ...
    """Plot 08: population over time for all 3 conditions under selected ecology."""
    ...
    # Right: key metrics bar chart (extinction rate, max_gen, init_child_surv)
    ax2 = axes[1]
    parts = sel_eco_key.split("_")
    ifv_i = int(parts[0][1:])
    ra_i  = int(parts[1][1:])
    ...
    x = np.arange(3)
    width = 0.25
    ax2.bar(x - width, ext_vals, width, label="Extinction Rate",  color=[COLORS_COND[c] for c in cond_order], alpha=0.5)
    ax2.bar(x,          [v / 15 for v in mg_vals], width, label="Max Gen / 15 (scaled)", color=[COLORS_COND[c] for c in cond_order], alpha=0.75)
    ax2.bar(x + width,  cs_vals, width, label="Init Child Surv.", color=[COLORS_COND[c] for c in cond_order], edgecolor="black", linewidth=0.6)
    ax2.set_xticks(x)
    ...
    fig.suptitle(
        f"Phase 5b: Selected Ecology — init_food={ifv_i}, replenish={ra_i}\n"
        f"No-Care vs Canonical vs High-Care",
        fontsize=11,
    )
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "plots", "08_condition_comparison_selected.png"), dpi=150)
    plt.close()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Phase 5b -- Food Ecology Calibration")
    parser.add_argument("--duration", type=int, default=3000)
    parser.add_argument("--seeds",    type=int, default=30)
    args = parser.parse_args()

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_dir   = os.path.join(PROJECT_ROOT, "outputs", "phase5b_food_ecology", timestamp)
    ...
    all_agg: list[dict] = []
    # all_per_tick[eco_key][condition_label] = aggregated per-tick list
    ...
    verdict = save_summary_json(all_agg, selected, out_dir)

    print("\nGenerating plots...")
    plot_pop_over_time_canonical(all_per_tick, out_dir, args.duration)
    plot_final_pop_heatmap(all_agg, out_dir)
