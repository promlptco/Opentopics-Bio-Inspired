# experiments/phase6c_biology_test/run_multi_seed.py
"""
Phase 6c — multi-seed runner (10 seeds, Options A+D combined).

Tests whether combined distress_sensitivity (cortisol) + care_recovery (prolactin)
rescues the population at MLE=1.25 where Phase 6a (plasticity-only) failed.

Baldwin classification per seed:
  care_recovered : final_cw - min_cw >= 0.05
  lr_swept       : final_lr - 0.10   >= 0.03
  ds_risen       : final_ds - 0.10   >= 0.05  (distress_sensitivity selected up)
  cr_risen       : final_cr - 0.10   >= 0.05  (care_recovery selected up)
...
def _run_seed_worker(packed):
    ...
    from experiments.phase6c_biology_test.run import run_phase6c, _pearson_r

    out_dir   = run_phase6c(seed=seed)
...
from experiments.phase6c_biology_test.run import (
    run_phase6c, _pearson_r, MULT, INIT_CARE, INIT_DS, INIT_CR,
)
...
COMBINED_DIR = os.path.join(PROJECT_ROOT, "outputs", "phase6c_biology_test", "multi_seed")
...
def _plot_trajectories(all_snaps, seeds, summaries, output_dir):
    ...
    fig.suptitle(
        f"Phase 6c — Options A+D: distress_sensitivity + care_recovery\n"
    ...
    ax.plot(common_ticks, care_mean, color="#2ca02c", linewidth=2.2,
            label=f"Phase 6c mean ± 95% CI  (n={len(all_snaps)} seeds)")
    ...
    # Panel 2: distress_sensitivity (Option A)
    ax = axes[1]
    for snaps in all_snaps:
        if snaps:
    ...
    # Panel 3: care_recovery (Option D)
    ax = axes[2]
    for snaps in all_snaps:
        if snaps:
            ax.plot([s["tick"] for s in snaps],
    ...
    # Panel 4: learning_rate (assimilation signal)
    ax = axes[3]
    ...
    fig.tight_layout()
    path = os.path.join(output_dir, "phase6c_care_trajectory.png")
...
def _plot_pearson_r(summaries, output_dir):
    ...
    ax.axhline(_mean(r_vals), color="seagreen", linestyle="--", linewidth=1.4,
               label=f"Phase 6c mean r = {_mean(r_vals):+.4f}")
    ...
    ax.set_title(
        "Phase 6c — Selection Gradient per Seed (Options A+D)\n"
        "Green = positive (care rising) | Red = negative (care eroding)"
    )
    ax.legend(loc="lower right", frameon=True)
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    path = os.path.join(output_dir, "phase6c_pearson_r.png")
    fig.savefig(path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved: {path}")
...
def run_all(seeds=SEEDS, max_workers=None):
    ...
    if max_workers > 1:
        print(f"\nPhase 6c multi-seed ({len(seeds)} seeds) — PARALLEL (workers={max_workers})")
        ...
        print(f"\nPhase 6c multi-seed ({len(seeds)} seeds): {seeds}")
        print(f"  Options A+D: distress_sensitivity={INIT_DS}, care_recovery={INIT_CR}")
        print(f"  Ecology: mult={MULT}, scatter=2, care_init={INIT_CARE}")
        if done:
    ...
    print("\n=== Phase 6c Multi-Seed Summary (Options A+D) ===")
