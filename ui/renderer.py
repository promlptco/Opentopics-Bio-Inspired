# ui/renderer.py
"""Renderer for full maternal simulation (mothers + children). Theme-driven via RendererTheme."""
import pygame
...
class Renderer:
    ...
    def render(
        ...
        for mother in mothers:
            ...
            if child:
                pygame.draw.line(self.screen, t.link_color,
                                 (mother.x * cs + cs // 2, mother.y * cs + cs // 2),
                                 (child.x * cs + cs // 2, child.y * cs + cs // 2), 1)
        ...
        for child in children:
            ...
            cy = child.y * cs + cs // 2
            radius = cs // 4
            body_color = lerp_color(1.0 - child.distress, t.child_color_low, t.child_color_high)
            pygame.draw.circle(self.screen, body_color, (cx, cy), radius)
            pygame.draw.circle(self.screen, t.outline_color, (cx, cy), radius, width=1)
        ...
        text = f"Tick: {tick}  |  Mothers: {alive_m}  |  Children: {alive_c}"
