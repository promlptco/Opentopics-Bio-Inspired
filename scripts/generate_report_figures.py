...
# Extinction ticks per condition (hardcoded from summary JSONs — max_pop=2500, mother_max_age=1000)
EXT_TICKS = {
    "mut_off_plast_off": [5763, 8648, 7532, 8294, 4816, 4284, 6776, 10367, 8394, 15182],
    "mut_on_plast_off":  [10057, 12341, 17894, 7760, 32423, 20377, 13737, 21707, 26238, 25191],
    "mut_off_plast_on":  [7459, 15211, 11034, 18318, 11724, 25400, 20589, 26255, 10581, 24344],
    "mut_on_plast_on":   [13400, 11245, 13724, 19543, 12831, 16607, 13022, 25446, 10267, 21215],
}
...
# ─────────────────────────────────────────────────────────────────────────────
def fig_ph2_ovat():
    # OVAT swept from the calibrated Balanced operating point:
    # init_food=40, eat_gain=0.50, move_cost=0.02, α=0.02
    PH2_OVAT = Path("outputs/phase2_survival_minimal/sensitivity/20260520_141958")

    # Balanced reference values for vertical marker lines
    BALANCED = {"init_food": 40, "eat_gain": 0.50, "move_cost": 0.02, "food_entropy_alpha": 0.02}
    REF_COLOR = "#C44E52"

    panels = [
        ("set_A_init_food.csv",          "init_food",          "Initial food cells",    "(a) Food abundance (init_food)"),
        ("set_B_eat_gain.csv",           "eat_gain",           "Energy gain per food",  "(b) Energy per food (eat_gain)"),
        ("set_C_move_cost.csv",          "move_cost",          "Energy cost per move",  "(c) Movement cost (move_cost)"),
        ("set_D_food_entropy_alpha.csv", "food_entropy_alpha", "Entropy parameter α",   "(d) Food patchiness (α)"),
    ]
    ...
    with plt.rc_context(STYLE):
        ...
        for ax, (fname, key, xlabel, title) in zip(axes.flat, panels):
            ...
            ax.fill_between(x, np.maximum(0, y - sd), y + sd, color="#4C72B0", alpha=0.18)

            # Vertical reference line at the selected Balanced value
            ref = BALANCED[key]
            ax.axvline(ref, color=REF_COLOR, linewidth=1.4, linestyle="--", alpha=0.85,
                       label=f"Balanced = {ref}")
        ...
        fig.suptitle("OVAT Sensitivity: Parameter Effect on Population Stability\n"
                     "(all other parameters held at Balanced operating point)",
...
def fig_food_mechanism():
    PH3_JSON = Path("outputs/phase3_food_comparison/exp_20260520_150834/summary.json")
    ...
    orig_pop    = [ph3[c]["orig_tail_pop"]          for c in conds]
    orig_pop_sd = [ph3[c]["orig_tail_pop_sd"]       for c in conds]
    orig_nrg    = [ph3[c]["orig_tail_energy"]       for c in conds]
    orig_nrg_sd = [ph3[c]["orig_tail_energy_sd"]    for c in conds]
    matr        = [ph3[c]["child_maturation_rate_mean"] for c in conds]
    matr_sd     = [ph3[c]["child_maturation_rate_sd"]   for c in conds]
    ...
    with plt.rc_context(STYLE):
        ...
        # ── (a) Original mother survival count ───────────────────────────────
        ax = axes[0]
        bars = ax.bar(x, orig_pop, w, yerr=orig_pop_sd, color=colors, alpha=0.82, **ekw)
        ax.axhline(15, color="#888888", linestyle="--", linewidth=1.0, label="Initial 15 mothers")
        for bar, v, sd in zip(bars, orig_pop, orig_pop_sd):
            ax.text(bar.get_x() + bar.get_width()/2, v + sd + 0.3, f"{v:.1f}",
                    ha="center", va="bottom", fontsize=10)
        ax.set_xticks(x); ax.set_xticklabels(alpha_labels)
        ax.set_ylabel("Mean alive original mothers (tail window)")
        ax.set_xlabel("Food distribution condition")
        ax.set_title("(a) Original mother survival")
        ax.set_ylim(0, 18)
        ax.legend(fontsize=8.5, loc="lower right")

        # ── (b) Original mother mean energy ──────────────────────────────────
        ax = axes[1]
        bars = ax.bar(x, orig_nrg, w, yerr=orig_nrg_sd, color=colors, alpha=0.82, **ekw)
        ax.axhline(1.0, color="#888888", linestyle="--", linewidth=0.9)
        for bar, v, sd in zip(bars, orig_nrg, orig_nrg_sd):
            ax.text(bar.get_x() + bar.get_width()/2, v + sd + 0.01, f"{v:.3f}",
                    ha="center", va="bottom", fontsize=10)
        ax.set_xticks(x); ax.set_xticklabels(alpha_labels)
        ax.set_ylabel("Mean energy of original mothers (tail window)")
        ax.set_xlabel("Food distribution condition")
        ax.set_title("(b) Original mother energy")
        ax.set_ylim(0, 1.15)

        # ── (c) Child maturation rate ─────────────────────────────────────────
        ax = axes[2]
        bars = ax.bar(x, matr, w, yerr=matr_sd, color=colors, alpha=0.82, **ekw)
        ax.axhline(1.0, color="#999999", linestyle="--", linewidth=0.9)
        for bar, v, sd in zip(bars, matr, matr_sd):
            ax.text(bar.get_x() + bar.get_width()/2, v + sd + 0.02, f"{v:.3f}",
                    ha="center", va="bottom", fontsize=10)
        ax.set_xticks(x); ax.set_xticklabels(alpha_labels)
        ax.set_ylabel("Child maturation rate (fraction)")
        ax.set_xlabel("Food distribution condition")
        ax.set_title("(c) Child maturation rate")
        ax.set_ylim(0, 1.22)
...
# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 3a ── Motivation Domain Winner per Food Condition
# ─────────────────────────────────────────────────────────────────────────────
def fig_food_motivation():
    PH3_JSON = Path("outputs/phase3_food_comparison/exp_20260520_150834/summary.json")
    with open(PH3_JSON) as f:
        ph3 = json.load(f)["conditions"]
    ...
    forage_m = [ph3[c]["forage_pct_mean"] for c in conds]
    forage_s = [ph3[c]["forage_pct_sd"]   for c in conds]
    care_m   = [ph3[c]["care_pct_mean"]   for c in conds]
    care_s   = [ph3[c]["care_pct_sd"]     for c in conds]
    self_m   = [ph3[c]["self_pct_mean"]   for c in conds]
    self_s   = [ph3[c]["self_pct_sd"]     for c in conds]

    panels = [
        (forage_m, forage_s, "(a) Forage motivation", "Fraction of decisions won by FORAGE", "#DD8452"),
        (care_m,   care_s,   "(b) Care motivation",   "Fraction of decisions won by CARE",   "#55A868"),
        (self_m,   self_s,   "(c) Self motivation",   "Fraction of decisions won by SELF",   "#8172B2"),
    ]
    ...
    with plt.rc_context(STYLE):
        ...
        for ax, (vals, sds, title, ylabel, color) in zip(axes, panels):
            bars = ax.bar(x, vals, w, yerr=sds, color=color, alpha=0.85, edgecolor="white", **ekw)
            top = max(vals) * 1.25 if max(vals) > 0 else 0.5
            for bar, v, sd in zip(bars, vals, sds):
                ax.text(bar.get_x() + bar.get_width() / 2, v + sd + top * 0.025,
                        f"{v:.3f}", ha="center", va="bottom", fontsize=10, fontweight="bold")
            ax.axhline(1 / 3, color="#888888", linestyle="--", linewidth=1.0, alpha=0.7,
            ...
            ax.legend(fontsize=9)
    savefig(fig, "fig03a_food_motivation.png")
...
# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 8c ── Genome vs Expressed: ALL 4 conditions × 3 motivations (4×3 grid)
# ─────────────────────────────────────────────────────────────────────────────
def fig_ph5_expressed_vs_genome_4cond():
    """4-row × 3-col panel: genome vs expressed weight for every
    condition × motivation combination.
    Plast OFF rows show expressed ≈ genome (no gap).
    Plast ON rows show systematic suppression of care/self.
    Useful to compare which mechanism drives the genome-expressed gap.
    """
    motives = [
        ("mean_genome_care",   "mean_expressed_care",   "#2166ac", "#d6604d", "Care  (g_c / w_c)"),
        ("mean_genome_forage", "mean_expressed_forage", "#1a9850", "#f4a582", "Forage  (g_f / w_f)"),
        ("mean_genome_self",   "mean_expressed_self",   "#7b2d8b", "#fdae61", "Self  (g_s / w_s)"),
    ]
    ...
    with plt.rc_context(STYLE):
        ...
        for row, (cond, folder) in enumerate(COND_DIRS):
            df = pd.read_csv(BASE / f"outputs/phase5_evolution/{folder}/snapshots.csv")
            df = df[df["n_mothers"] > 0]

            for col, (g_col, e_col, g_color, e_color, m_title) in enumerate(motives):
                ax = axes[row, col]
                agg = df.groupby("tick")[[g_col, e_col]].agg(["mean", "sem"]).reset_index()
        ...
        fig.suptitle(
            ...
            fontsize=13, fontweight="bold")
    savefig(fig, "fig08c_ph5_expressed_vs_genome_4cond.png")


# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 21 ── Why Did All Lineages Go Extinct? (energy + diversity cascade)
# ─────────────────────────────────────────────────────────────────────────────
def fig_extinction_mechanism():
    """4-panel decomposition of the extinction cascade.

    (a) Population trajectory — all 4 conditions; shows boom-bust dynamics
        and the ecological carrying capacity ceiling (~150-175 agents).
    (b) Mean mother energy over time — declining baseline shows resource
        exhaustion as population peaks and food is depleted faster than
        Shannon respawn can recover it.
    (c) Genome diversity (std_genome_care) over time — shows diversity
        buildup and then collapse near extinction, removing the raw material
        for selection. Mutation-OFF lines stay near zero throughout.
    (d) Energy budget breakdown for the two plast-ON conditions —
        mean_learning_cost vs mean_mother_energy shows how much of the
        energy budget the plasticity maintenance cost consumes.
    """
    frames = {}
    for cond, folder in COND_DIRS:
        df = pd.read_csv(BASE / f"outputs/phase5_evolution/{folder}/snapshots.csv")
    ...
    CARRY = 165  # approximate ecological carrying capacity
    ...
    with plt.rc_context(STYLE):
        ...
        for cond, folder in COND_DIRS:
            df = frames[cond]
            agg = df.groupby("tick")["population_size"].agg(["mean", "sem"]).reset_index()
            ax_a.plot(agg["tick"] / 1000, agg["mean"],
                      color=COND_COLORS[cond], lw=1.8, label=COND_LABELS[cond])
            ax_a.fill_between(agg["tick"] / 1000,
        ...
        # ── (b) Mean mother energy — resource exhaustion signal ───────────────
        for cond, folder in COND_DIRS:
            df = frames[cond]
            dfa = df[df["n_mothers"] > 0]
            agg = dfa.groupby("tick")["mean_mother_energy"].agg(["mean", "sem"]).reset_index()
            ax_b.plot(agg["tick"] / 1000, agg["mean"],
                      color=COND_COLORS[cond], lw=1.8, label=COND_LABELS[cond])
            ax_b.fill_between(agg["tick"] / 1000,
                              (agg["mean"] - agg["sem"]).clip(0),
                              agg["mean"] + agg["sem"],
                              color=COND_COLORS[cond], alpha=0.13)
        ...
        # ── (c) Genome diversity (std_genome_care) — diversity collapse ───────
        for cond, folder in COND_DIRS:
            df = frames[cond]
            dfa = df[df["n_mothers"] > 0]
            agg = dfa.groupby("tick")["std_genome_care"].agg(["mean", "sem"]).reset_index()
            ax_c.plot(agg["tick"] / 1000, agg["mean"],
                      color=COND_COLORS[cond], lw=1.8, label=COND_LABELS[cond])
            ax_c.fill_between(agg["tick"] / 1000,
                              (agg["mean"] - agg["sem"]).clip(0),
                              agg["mean"] + agg["sem"],
                              color=COND_COLORS[cond], alpha=0.13)
        ...
        # ── (d) Learning cost vs mother energy: plasticity tax ─────────────────
        plast_conds = [("mut_on_plast_on",  "block2_main_mut_on_plast_on"),
                       ("mut_off_plast_on", "block2_main_mut_off_plast_on")]
        ref_conds   = [("mut_on_plast_off",  "block2_main_mut_on_plast_off")]

        for cond, folder in plast_conds + ref_conds:
            df = frames[cond]
            dfa = df[df["n_mothers"] > 0]
            agg_e = dfa.groupby("tick")["mean_mother_energy"].agg(["mean", "sem"]).reset_index()
            agg_c = dfa.groupby("tick")["mean_learning_cost"].agg(["mean", "sem"]).reset_index()

            ls = "-" if "plast_on" in cond else "--"
            ax_d.plot(agg_e["tick"] / 1000, agg_e["mean"],
                      color=COND_COLORS[cond], lw=1.8, linestyle=ls,
                      label=f"{COND_LABELS[cond]} energy")
            ax_d.plot(agg_c["tick"] / 1000, agg_c["mean"],
                      color=COND_COLORS[cond], lw=1.4, linestyle=":",
                      alpha=0.7, label=f"{COND_LABELS[cond]} learning cost")
        ...
        fig.suptitle(
            ...
            fontsize=13, fontweight="bold")
    savefig(fig, "fig21_extinction_mechanism.png")


# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 22 ── Why Mut ON / Plast ON < Mut ON / Plast OFF?
